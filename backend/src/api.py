...
def get_all_drinks():
    try:
        ...
        # if drinks is null, return 404 error
        if drinks is None:
            abort(404)
...
def update_drink(payload, id):
    ...
    try:
        ...
        # if drink with <id> is not found, return 404 error
        if drink is None:
            abort(404)
...
def auth_error(error):
    return jsonify({"success": False, "error": error.status_code, "message": error.error['description']
    }), error.status_code
